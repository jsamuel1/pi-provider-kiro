...
Repair does not relocate a tool result away from the assistant it sits behind, and does not repair `ALTERNATING_MESSAGES`. Both limits are pinned by tests in `test/stream.test.ts`; see the CHANGELOG entry for why.
